...
export async function POST(request: NextRequest) {
  ...
  try {
    runId = await createScrapeRun('euro-auctions');

    const rawItems = await scrapeEuroAuctions({ maxPages: 3 });
    ...
    const deactivatedCount = await deactivateStale('euro-auctions', activeLotIds);
